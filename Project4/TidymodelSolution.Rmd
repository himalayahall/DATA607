...
Let's extract the number of available cores. We use this during model setup.
...
model <- rand_forest() %>%
  set_mode("classification") %>%
  set_engine(engine = "ranger", num.threads = 1 + num_cores / 2, importance = "impurity")
...
So far we have been keeping the test split in our back pocket. Now it's time to use the test data to evaulate our final model. Let's do the final fit - train with full training data and test with testing data.
