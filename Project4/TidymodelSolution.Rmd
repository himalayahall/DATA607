...
In this markdown we will use [tidymodels](https://www.tidymodels.org) to build ham/spam classifiers for SMS messages. The library provides a framework for modeling and machine learning using [tidyverse](https://www.tidyverse.org) principles. 
...
Set up feature engineering recipe to get the data ready mo modeling. We set it up to predict **cat** (outcome) from **sms** (predictor). The predictor is tokenized, stop words are removed, and tokens are filtered (max number of tokens retained and min times a token must appear to be considered at all). Tokens are encoded using **tf-idf**. Finally, data is down-sampled such that the majority class level will have (at most) twice as many rows as the minority level.
...
```{r}
# model <-
#   mlp(hidden_units = 256, dropout = 0.1, epochs = 50) %>%
