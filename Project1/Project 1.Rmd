...
Since we will be extracting rows/columns we can convert to matrix format for easier downstream processing.
...
Let's convert all data to uppercase to ensure  mixed casing in the input will not break downstream processing.

```{r}
input_matrix <- map(input_matrix, 
                   function(x) str_to_upper(x))
```

Observe that first 3 rows are headers and do not contain player info.

```{r}
head(input_matrix, n = 10)
```

Using this observation we can skip the 3 header rows.
...
Our next observation is that data for each player is provided on 2 consecutive rows, followed by a dashed separator line. 1^st^ row contains the player name, total points, and games played by them. The 2^nd^ row contains the State and initial rank of the player.  

Using this observation, let's split the input matrix into 2 components.  
...
mPlayersAndGames <- str_replace_all(mPlayersAndGames, "\\|([BHXFU])([ \t\f\n])+", "|\\1\\2 \\N\\A")
```
...
aditya_id <- 3               # ADITYA's id

mPlayersAndGames[aditya_id]  # ADITYA's games

p_opponents[aditya_id]       # ADITYA's opponents
```
...
Let's validate our transformations. As expected, the counst of games with opponent ids for KENNETH J TACK, VIRAJ MOHILE, and ASHWIN BALAJI are 6, 6, and 1, respectively.  
...
As a find sanity check, verify that key data frame values for KENNETH match earlier validations.

```{r}
opp_avg_prerating[[kenneth_idx]] == result_df[kenneth_idx, "avg_opponent_pre_rating"]

player_pre_rating[kenneth_idx] == result_df[kenneth_idx, "pre_rating"]
