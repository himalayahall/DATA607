...
First load the JSON file. As expected, the resulting data frame contains 3 observatios of 5 variables. Note, the *pages* variable is of *int* type.
...
Next, we load the same data in XML format. Note, both JSON and XML data frames are very similar. However, unlike the JSON version, the *pages* data type is *chr* (string).
...
XML provides support for data types through XML Schemas. Let's load the same XML data that contains data types.
...
Finally, the same data is loaded in HTML format. Here, the data frame is 
